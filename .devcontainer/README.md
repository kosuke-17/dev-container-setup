...
> **これは「完全に安全」な環境ではない。** 残存リスクがある（後述）。
> 何が防げていて何が防げていないかは `ai-reviews/` の2本のレビューを必ず読むこと。
> 2026-07-21 の再レビューで指摘された High-1 / High-2 は 2026-08-09 に修正済み。
...
**fail-close**: 途中で失敗した場合は `fail_close` がポリシーDROPに加えて**ルールもフラッシュ**する
（ポリシーDROPだけでは残存ACCEPTルールに勝てないため）。エラー終了はすべて `die` 経由に統一してあり、
`exit 1` では ERR trap が発火しないという bash の仕様による取りこぼしを避けている。

**ipset構築中の一時許可**: 許可リストを組み立てるまでの数秒間だけ HTTPS(443) を開けるが、
`-m owner --uid-owner 0` で **root プロセス限定**にしている。これがないと、node ユーザー
（= Claude 自身）が `sudo init-firewall.sh` を再実行するだけで全開放ウィンドウを任意に再現できてしまう。
`xt_owner` が使えない環境では警告を出して全開放にフォールバックする（その場合はこの穴が残る）。

`--cap-add=NET_ADMIN` / `--cap-add=NET_RAW`（`runArgs`）は、このfirewall構築（iptables/ipset操作）に必要な権限。

## 既知の残存リスク（重要）

`ai-reviews/2026-07-21-devcontainerセキュリティ再レビュー.md` の High-1（不完全な fail-close）と
High-2（443 一時全開放ウィンドウ）は 2026-08-09 に修正済み。以下は**修正できていない**:

- **DNS トンネリング**: リゾルバ宛に限定しても、再帰リゾルバ経由で攻撃者の権威サーバへは到達する
  （iodine 等）。帯域は細いが「塞げた」と認識しないこと。
